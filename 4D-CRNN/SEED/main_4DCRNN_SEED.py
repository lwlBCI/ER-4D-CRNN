...
falx = np.load('G:/For_EEG/EEG_code/4 （论文加代码）基于CNN和LSTM的脑电情绪识别（数据集为DEAP和seed）4D-CRNN/4D-CRNN-master/SEED/DE0.5s/t6x_89.npy')
y = np.load('G:/For_EEG/EEG_code/4 （论文加代码）基于CNN和LSTM的脑电情绪识别（数据集为DEAP和seed）4D-CRNN/4D-CRNN-master/SEED/DE0.5s/t6y_89.npy')

# one_y = np.array([y[:1697]] * 3).reshape((-1,))
# one_y = to_categorical(one_y, num_classes)
one_y_1 = np.array([y[:1126]] * 3).reshape((-1,))
one_y_1 = to_categorical(one_y_1, num_classes)  # one_y_1.shape=(3378*3)，因为每位受试者做了三次实验所以是3378=1126*3，后面的3是独热编码的结果
# 并且15位受试者的实验顺序和实验内容完全相同，所以这个标签可以重复使用
...
for nb in range(15):
    K.clear_session()
    start = time.time()
    one_falx_1 = falx[nb * 3:nb * 3 + 3]  # 取出每位受试者的三次实验样本,每个人的三次样本数据是挨在一起的
    one_falx_1 = one_falx_1.reshape((-1, 6, img_rows, img_cols, 5))
    # print("one_falx_1.shape:",one_falx_1.shape) (3378,6,8,9,5)
    # ###============= random select ============####
    ...
    # ###============= random select ============####
    one_y = one_y_1
    one_falx = one_falx_1[:,:,:,:,1:5]  # 这里跟deap数据集一样，只用了四个频段

    # print(one_y.shape)     (3378,3)
    # print(one_falx.shape)  (3378,6,8,9,4)
